...
# === CONFIG ===
base_folder = 'object_images/MI3DOR'
output_json_path = 'object_database/MI3DOR/descriptions_attributes.json'
...
# === Prompt ===
prompt = (
    "Extract visual attributes of the object in the image: object type, brand name, color, material, and label text."
)
...
##caption_prompt = ""Write a detailed visual caption of the object, mentioning colors, materials, brand names, and visible labels."
...
for folder_name in sorted(os.listdir(base_folder)):
    ...
    # Determine which image to describe
    if "monitor" in folder_name.lower():  # <-- Adjust condition if needed
        target_file = f"{folder_name}_011.png"
    else:
        target_file = f"{folder_name}_002.png"

    image_path = os.path.join(folder_path, target_file)

     # Skip if already processed
    if target_file in image_descriptions:
        print(f'🔁 Skipping already-described image: {target_file}')
        caption = image_descriptions[target_file]
    else:
        if not os.path.exists(image_path):
            print(f"❌ Expected file not found: {target_file}. Skipping folder.")
            continue

        print(f'🖼️  Image: {target_file}')
        image = Image.open(image_path).convert("RGB")
        caption = generate_caption(image, prompt)

        image_descriptions[target_file] = caption
        print(caption)
