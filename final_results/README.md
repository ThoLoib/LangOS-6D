...
| Referenz-Hardware aller Laeufe (Pulli-identisch: Ryzen 9 5900X + RTX 4090) | `hardware.json` | lscpu/nvidia-smi/free/uname (Host) + Gegenprobe im oscar-Container, 2026-09-25 |
| 4.1 Anfrage | `stage4/query_latency_ycbv_geo_pose.json` (**Geometrie UND Pose auf denselben 50 Queries**, frischer dGeDi-Container; Manifest mit den vier Per-Query-Summen Retrieval/+Geo/+Pose/+Geo+Pose), `stage4/query_latency_ycbv.json` (partial, ohne Geometrie), `…_fullmesh.json`, `…_geo.json` (Geometrie ohne Pose, n=25) | `experiments/experiment4_query_latency.py` (Records-`total_s` seit 2026-09-25 ohne Doppelzaehlung der Klammer `retrieval_total`) |
| 4.2 Onboarding | `stage4/onboarding.json` (n=59, partial), `stage4/onboarding_render_n59.json` (Render-Vollerhebung), `stage4/onboarding_render.json` (alte n=5-Messung, als Beleg der Korrektur), `stage4/onboarding_dgedi.json` (n=3, KALT: je Objekt eigener docker-compose-Aufruf inkl. Container-/Python-/Modellstart), `stage4/onboarding_dgedi_warm_n59.json` (**n=59 wie die uebrige Onboarding-Tabelle**, WARM: median 1.414 s je Objekt, je Datensatz aufgeschluesselt; einmaliger Start getrennt), `stage4/onboarding_dgedi_warm.json` (n=21, nur YCB-V — Vorstufe), `stage4/onboarding_warm_render_describe.json` (**WARM render+describe, n=59**: Blender-Sitzung einmal je Datensatz, LLaVA einmal geladen; Batch 8 vs 1; Befund zu den alten describe-Kaltwerten im Manifest) | `scripts/stage4_onboarding.sh`, `scripts/run_stage4_render_full.sh`; warm: `dgedi_service/precompute_gallery.py --timing-json` (99180da9) |
