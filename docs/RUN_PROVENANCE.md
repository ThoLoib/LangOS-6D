...
| Stage-1-Gewichts-Sweep v2 (`weight_sweep_66_pc.csv`, `weight_sweep_66_cross.csv`) | 25.09. 09:16 (pc) / 10:22 (cross) | `experiments/stage1_weight_sweep_v2.py` (Commit-Stand 5eb9574d-Vorlauf) | ✓ | — | — | **Produktions-Config 42v/k5** (SHAPE_AGG_VIEWS=42, ulip_view_topk=5, DINO 42v, partielle Referenzen), GEOM_K=5; nutzt AblationSpec/make_fusion_module/derive_ranking auf den Score-Stores `_cache/scores_*.pt` (kein GPU-Lauf). Wächter: BASE pc == E1c_full_fusion 0.5868/0.3413 EXAKT, BASE cross == E7_ulip2_cross 0.5588/0.3289 EXAKT (n=2101). Ersetzt die 16v/k8-Weightmaps v1 (nur historisch) |
| `query_latency_ycbv_geo_pose.json` | 25.09. 15:43–15:57 | `experiment4_query_latency.py` direkt (compose run oscar, PYTHONHASHSEED=0) | `--dataset ycbv --n-queries 50 --views 16,42 --geometry --geo-k 5` (Pose an, refine-iter 5, Seed 0) | — | — | dGeDi-Container FRISCH (--force-recreate 15:41, zuvor 7 Tage Dauerlast — Erstmessung 5.45 s dadurch erklaert; frisch: 1.78/1.85 s). Query-Sequenzen identisch zum 04.09.-Lauf (Seed-Reproduktion geprueft). Records-total_s-Doppelzaehlung gefixt |
| Referenz-Hardware (`final_results/hardware.json`) | 25.09. erhoben | lscpu/nvidia-smi/free/uname Host + Gegenprobe im oscar-Container | — | — | — | AMD Ryzen 9 5900X (12C/24T), RTX 4090 24564 MiB, Treiber 555.42.06, torch 2.10.0+cu128/CUDA 12.8, Ubuntu 20.04.6, 125 GiB RAM. IDENTISCHE CPU+GPU wie Pulli et al. (OSCAR 2025) — Latenzvergleiche direkt zulaessig |
| Onboarding WARM: render+describe (`onboarding_warm_render_describe.json`) | 25.09. 17:10–19:05 | `experiments/stage4_warm_describe.py` (oscar-Container) + `experiments/stage4_warm_render.py` (Host-Blender 3.4.1) | 59 Ziel-CADs x {16,42} Views; describe Batch 8 UND Batch 1; RENDER_TIMING_JSON-Hook in rendering.py | — | — | GPU EXKLUSIV (dgedi+foundationpose gestoppt, danach neu gestartet); ein kontaminierter Render-Lauf verworfen. Waechter: Bilder 59x2 vollstaendig 512x512; Captions v42_b8 2478/2478 identisch, v16_b8 932/944 (Batch-Grenzen-Numerik). BEFUND: onboarding.json-describe (10.25/13.08 s) kann keine Vollbeschreibung gewesen sein (frischer Kalt-Nachweis 17.7/28.7 s) |
