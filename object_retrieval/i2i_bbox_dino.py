    ...
    for scene_id in tqdm(os.listdir(bop_root), desc="Scenes"):
        ...
        for img_id_str in tqdm(scene_gt.keys(), desc=f"Images in {scene_id}", leave=False):
            ...
            for obj_inst, info in zip(gt_instances, gt_infos):
                ...
                similarities = F.cosine_similarity(features, ref_embeddings.to(device))  # [num_refs]
...
if __name__ == "__main__":
    ...
    results_path = os.path.join(bop_data_root, "ycbv_results_dinov2.csv")
    ...
    compute_map_at_k(results, ks=[1, 3, 5, 10], save_topk_file="top10_rankings_dino_ycbv_gso.json")
