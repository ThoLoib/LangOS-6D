...
# === CONFIG ===
result_path = "txt2txt_attributes_results_ycbv.csv"
rgb_dirname = "rgb"
bop_root = '../eval/datasets/ycbv_test_bop19/test/'
description_file = '../object_database/ycbv/descriptions_attributes.json'
...
def evaluate_text_to_text_retrieval(
    caption_embeddings,  # numpy array (N_queries, D)
    desc_embeddings,     # numpy array (N_desc, D)
    desc_labels,         # list[str], len = N_desc
    caption_image_keys,  # list of tuples: (scene, img_id_str, idx, gt_id)
    object_to_id,        # dict: label -> id (inverted earlier), so id->label is:
    ks=[1, 3, 5, 10],
    save_csv=None,
    ...
    for k in ks:
        ...
        print(f"RA@{k}: {acc:.4f} ({topk_hits[k]}/{total})")
...
evaluate_text_to_text_retrieval(
    ...
    ks=[1, 3, 5, 10],
    save_csv="retrieval_text_results.csv",
    save_json="retrieval_top10.json"
)
