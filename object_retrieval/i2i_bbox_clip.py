...
results_dir = "results_clip_ycbv"
# =============== 

id_to_label_file = os.path.join(bop_data_root, "id_to_label.json")
        ...
        for fname in os.listdir(obj_path):
            if fname.lower().endswith(('.jpg', '.png')):
                ...
                with torch.no_grad():
                    ...
                    embeddings.append(features[0].cpu())
                    image_info.append((obj_folder, img_path))
...
def evaluate_image(image_path, box_path, ref_embeddings, ref_keys):
    ...
    for object_id, bbox in boxes:
        ...
        print(f"Top 5 similarities: {sorted(similarities.tolist(), reverse=True)[:5]}")
    ...
    for scene_id in tqdm(os.listdir(bop_root), desc="Scenes"):
        ...
        for img_id_str in tqdm(scene_gt.keys(), desc=f"Images in {scene_id}", leave=False):
            ...
            for obj_inst, info in zip(gt_instances, gt_infos):
                ...
                similarities = F.cosine_similarity(features, ref_embeddings.to(device))
...
if __name__ == "__main__":
    ...
    compute_map_at_k(results, ks=[1, 3, 5, 10], save_topk_file=json_file)
