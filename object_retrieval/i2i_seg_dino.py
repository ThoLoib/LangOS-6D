...
def evaluate_bop_dataset(bop_root, ref_embeddings, ref_keys, id_to_label):
    ...
    for scene_id in tqdm(os.listdir(bop_root), desc="Scenes"):
        ...
        if not (os.path.exists(scene_gt_path) and os.path.exists(scene_gt_info_path) and os.path.isdir(rgb_dir)):
            print(f"Warning: Missing data in {scene_dir}, skipping.")
            continue
        ...
        for img_id_str in tqdm(scene_gt.keys(), desc=f"Images in {scene_id}", leave=False):
            ...
            for inst_id, (obj_inst, info) in enumerate(zip(gt_instances, gt_infos)):
                obj_id = obj_inst["obj_id"]
                object_label = id_to_label.get(str(obj_inst["obj_id"]))
                if object_label is None:
                    print(f"Warning: Object ID {obj_id} not found in id_to_label mapping.")
                    continue

                mask_path = os.path.join(scene_dir, 'mask_visib', f"{img_id:06d}_{inst_id:06d}.png")
                ...
                crop_filename = os.path.basename(image_path).replace('.', f'_segm_{img_id}_{object_label}.')
                crop_path = os.path.join("segm", crop_filename)
                ...
                with torch.no_grad():
                    ...
                    features = F.normalize(features, p=2, dim=1)  # [1, D]
                    refs = F.normalize(ref_embeddings, p=2, dim=1)  # [N, D]
                    similarities = torch.mm(features, refs.T).squeeze(0)  # [N]

                best_idx = similarities.argmax().item()
                pred_label, best_ref_path = ref_keys[best_idx]

                # Top-5
                topk_values, topk_indices = torch.topk(similarities, k=min(5, len(similarities)))
                top5 = [(ref_keys[i][0], ref_keys[i][1], float(topk_values[idx])) for idx, i in enumerate(topk_indices)]
                ...
                all_results.append({
                    ...
                    "similarity_score": float(topk_values[0]),
...
def compute_map_at_k(results, ks=[1, 3, 5, 10], save_topk_file="topk_rankings.json"):
    ...
    for r in results:
        ...
        sorted_labels = sorted(sim_dict.items(), key=lambda x: x[1], reverse=True)
...
if __name__ == "__main__":
    ...
    results_dir = "results_dino_segm_hcat"
    ...
    metrics_txt = os.path.join(results_dir, "metrics_summary.txt")

    ref_embeddings, ref_keys = load_reference_image_embeddings(ref_dir)
    ref_embeddings = torch.stack(ref_embeddings).to(device)

    results, ref_keys = evaluate_bop_dataset(bop_data_root, ref_embeddings, ref_keys, id_to_label)
    
    #save_results_to_csv(results, results_path)
    compute_retrieval_metrics(results, ks=[1, 3, 5, 10], csv_filename=csv_file)

    compute_map_at_k(results, ks=[1, 3, 5, 10], save_topk_file="top10_rankings.json")
    #compute_map(results)